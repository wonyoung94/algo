...
def solution(numbers):
    ...
    while i < len(numbers): # i가 numbers 를 다 돌때까지
        for word in my_dict.keys(): # my_dict에 있는 keys를 다 확인해라
            if numbers[i:i + len(word)] == word: # i:i + len(word) => (ex .one: 3) 가 one과 같다면
                answer += str(my_dict[word]) # my_dict의 word를 str화 해서 answer에 넣어라
                i += len(word) # i는 len(word)만큼 더 이동한다(ex .one: 3) : 안할경우 1112223333344...가 됨
                break

    return int(answer) # 에쒸 원하는건 또 int 형이라 또 변환함 ㅋㅋㅋ
